            ...
            <div class="row">
                <div class="col-lg-6">
                    <ul class="tg-header__top-info left-side list-wrap">
                        ...
                        <li><i class="flaticon-pin"></i>{{ $setting['site_location'] ?? '' }}</li>
                    </ul>
                </div>
                <div class="col-lg-6">
...
$menus = getMenus(1);
                                    ...
                                    @if ($menus)
                                        @foreach ($menus as $menu)
                                            @php
        $mainClass = isset($menu->children) ? 'menu-item-has-children' : '';
                                            @endphp
                                            <li class="{{ $mainClass }}">
                                                <a href="{{ $menu->slug }}" target="{{ $menu->target ?? '_self' }}">
                                                ...
                                                @if (isset($menu->children))
                                                    <ul class="sub-menu">
                                                        @foreach ($menu->children as $childGroup)
                                                            @foreach ($childGroup as $child)
                                                                @php
                    $childClass = isset($child->children) ? 'menu-item-has-children' : '';
                                                                @endphp
                                                                <li class="{{ $childClass }}">
                                                                    <a href="{{ $child->slug }}" target="{{ $child->target ?? '_self' }}">
                                                                    ...
                                                                    @if (isset($child->children))
                                                                        <ul class="sub-menu">
                                                                            @foreach ($child->children as $subChildGroup)
                                                                                @foreach ($subChildGroup as $subChild)
                                                                                    @php
                                $subChildClass = isset($subChild->children) ? 'menu-item-has-children' : '';
                                                                                    @endphp
                                                                                    <li class="{{ $subChildClass }}">
                                                                                        <a href="{{ $subChild->slug }}" target="{{ $subChild->target ?? '_self' }}">
                                                                                            {{ $subChild->name ?? $subChild->title }}
                                                                                        </a>

                                                                                        {{-- Level 3 Children --}}
                                                                                        @if (isset($subChild->children))
                                                                                            <ul class="sub-menu">
                                                                                                @foreach ($subChild->children as $ssChildGroup)
                                                                                                    @foreach ($ssChildGroup as $ssChild)
                                                                                                        <li>
                                                                                                            <a href="{{ $ssChild->slug }}" target="{{ $ssChild->target ?? '_self' }}">
                                                                                                                {{ $ssChild->name ?? $ssChild->title }}
                                                                                                            </a>
                                                                                                        </li>
                                                                                                    @endforeach
                                                                                                @endforeach
                    ...
                    <div class="tgmobile__menu">
                        <nav class="tgmobile__menu-box">
                            ...
                            <div class="nav-logo">
                                <a href="/"><img src="{{ $setting['site_main_logo'] ? asset(get_media($setting['site_main_logo'])->fullurl) : '' }}"
                                    alt="{{ $setting['site_main_logo'] ? get_media($setting['site_main_logo'])->alt : 'Amazing Infosys' }}"></a></a>
                            </div>
    ...
    <div class="offCanvas__info">
        ...
        <div class="offCanvas__side-info mb-30">
            ...
            <div class="contact-list mb-30">
                ...
                <p>{{ $setting['site_phone_two'] ?? '' }}</p>
            </div>
            <div class="contact-list mb-30">
                <h4>Email Address</h4>
                <p>{{ $setting['site_email'] ?? '' }}</p>
                <p>{{ $setting['site_email_two'] ?? '' }}</p>
            </div>
        </div>
